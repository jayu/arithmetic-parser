...
const debugTree = (expressionTree) => {
  console.log(JSON.stringify(expressionTree, undefined, 2))
}

const evaluateValidate = (expression) => {
  const expressionTree = parse(expression)
  debugTree(expressionTree)
...
evaluateValidate(onlyBracket)
evaluateValidate('a-b+c-d')
evaluateValidate(equation)
evaluateValidate('(a*n)')

const evaluatedTree = evaluateTree(parse(onlyBracket))
